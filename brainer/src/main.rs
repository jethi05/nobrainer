...
fn main() {
    match config::config_einlesen() {
        Ok(cfg) => {
            println!("Test {}:", cfg.test_var);
            println!("safe file {}:", cfg.safe_files);
            println!("template {}:", cfg.template_files);
        }
        Err(e) => {
            eprintln!("Fehler beim Einlesen der Config");
        }
    }
    ...
    //wb_schreiben::datei_laden(); 

    file::file_generieren(&cfg);
}
